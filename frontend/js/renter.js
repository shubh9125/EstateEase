...
if (renterUser) {
    document.getElementById("info").innerHTML = `
        <h2>Welcome, ${renterUser.fname || "Renter"}</h2>
        <p class="small">Role: ${renterUser.role} • Verified: ${renterUser.isVerified ? "Yes" : "No"}</p>
    `;
}
...
async function loadMyInbox() {
    ...
    try {
        ...
        list.forEach(r => {
            ...
            div.innerHTML = `
                <h4 style="margin: 0;">${r.propertyId?.title || "Rental Inquiry"}</h4>
                <div class="small" style="margin: 5px 0;">Landlord: ${r.propertyId?.ownerId?.fname || "N/A"}</div>
                ...
                <div style="margin-top: 10px; padding: 12px; background: #eff6ff; border-radius: 10px; border-left: 4px solid #3b82f6;">
                    <strong>Landlord's Reply:</strong><br>
                ...
                <div style="margin-top: 10px;">
                    ...
                    <button class="primary small" onclick="sendFollowUp('${r._id}')">Send Reply to Landlord</button>
                </div>
                ` : ""}
            `;
            box.appendChild(div);
        ...
        if (!list.length) {
            box.innerHTML = `<div class="card">No inquiries sent yet.</div>`;
        }
    } catch (e) {
